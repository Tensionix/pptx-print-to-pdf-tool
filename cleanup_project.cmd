...
title Audion PPTX Print to PDF Tool - Cleanup
...
echo ======================================================================
echo   AUDION PPTX PRINT TO PDF TOOL - LEAN SOURCE CLEANUP
echo ======================================================================
echo Root:
echo   %BASE_DIR%
echo.
...
echo It removes managed user/work content, runtime payloads, build caches and generated outputs:
echo   - input, output, logs, report, state, data and release contents
echo   - runtime, wheelhouse, install\download contents
echo   - all folders named .runtime, _runtime, ._runtime, __pycache__
echo   - pytest/mypy/ruff caches and root build/dist folders
...
echo.
echo Protected: system_core source, config, docs, launchers, install scripts, release policies.
...
call :RemoveDir "%BASE_DIR%\input"
call :RemoveDir "%BASE_DIR%\output"
call :RemoveDir "%BASE_DIR%\logs"
call :RemoveDir "%BASE_DIR%\report"
call :RemoveDir "%BASE_DIR%\state"
call :RemoveDir "%BASE_DIR%\data"
call :RemoveDir "%BASE_DIR%\release"
call :RemoveDir "%BASE_DIR%\runtime"
call :RemoveDir "%BASE_DIR%\wheelhouse"
call :RemoveDir "%BASE_DIR%\install\download"
call :RemoveDir "%BASE_DIR%\system_core\powershell"
...
echo [OK] Managed folders were recreated after cleanup.
goto cleanup_done
...
if "%DRY_RUN%"=="1" (
  echo   [DRY] recursive search skipped for dry-run
  exit /b 0
)
