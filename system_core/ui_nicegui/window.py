...
def load_ui_defaults() -> dict[str, object]:
    try:
        ...
        return {
            "title": str(ui_info.get("title") or tool_info.get("name") or "Audion PPTX Print to PDF"),
            "host": str(ui_info.get("host") or "127.0.0.1"),
            "port": int(ui_info.get("port") or 8094),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Audion PPTX Print to PDF GUI window.")
